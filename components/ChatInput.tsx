      ...
      <div className="max-w-md w-full mx-auto flex flex-col items-center gap-6 pointer-events-auto relative">
        ...
        <div className={`transition-all duration-700 transform ${isConnected || isConnecting ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
           <span className="text-xs font-medium tracking-[0.2em] text-slate-400 uppercase font-sans flex items-center gap-2 bg-white/50 px-4 py-1 rounded-full backdrop-blur-md border border-white/50 shadow-sm">
              {isConnecting ? (
                  ...
                  <>
                    <span className="w-1.5 h-1.5 bg-santa-500 rounded-full animate-pulse"></span>
                    Live Interview
                  </>
              )}
           </span>
        ...
        <div className="relative group flex items-center justify-center">
            ...
            {!isConnected && !isConnecting && (
                <div className="absolute inset-0 bg-santa-500/5 rounded-full blur-xl animate-[pulse_4s_ease-in-out_infinite] scale-150"></div>
            )}

            {/* Active Glow - Red Only */}
            {isConnected && (
                <div className="absolute inset-0 bg-gradient-to-tr from-santa-400 to-santa-500 rounded-full blur-2xl opacity-40 animate-pulse"></div>
            )}
            ...
            <button 
                ...
                className={`relative w-24 h-24 rounded-full flex items-center justify-center transition-all duration-700 cubic-bezier(0.34, 1.56, 0.64, 1) backdrop-blur-xl border border-factory-500/20 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] hover:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.15)] hover:scale-105 active:scale-95 ${
                    isConnected
                    ? 'bg-paper/90 text-santa-600' 
                    : 'bg-paper/80 text-slate-700 hover:text-santa-600'
                }`}
            >
                ...
                {isConnected ? (
                     <div className="absolute inset-0 flex items-center justify-center gap-1">
                        {visualData.slice(0, 5).map((h, i) => (
                            <div 
                                key={i}
                                className="w-1 bg-santa-400 rounded-full transition-all duration-75 ease-linear opacity-80"
                    ...
                    isConnecting ? (
                        <div className="w-8 h-8 border-2 border-factory-300 border-t-santa-500 rounded-full animate-spin"></div>
                    ) : (
